...
MJPG_Streamer.prototype = {
	...
	shutdown: function() {
		sh('kill $(ps aux | grep -v grep |grep mjpg | grep -v sh | awk \'{print $2}\')');
	},
	tryRun: function(resetCamera) {
		...
		if (this.isAvailable()) {
			if (!this.isMjpgRunning() || resetCamera) {
				...
				if (this.formatName == 'YUVU' && this.options.quality == 'auto')
					this.options.quality = '65';
				else if (this.formatName == 'MJPEG' && this.options.quality == 'auto')
				...
				if (this.formatName == 'YUVU' && this.options.fps == 'auto')
					this.options.fps = '5';
				else if (this.formatName == 'MJPEG' && this.options.fps == 'auto')
					this.options.fps = '30';
