...
function Controller(options) {
	...
	this.token = new Token(this.render, {
		privateApiKey: options.ionicKey.privateApiKey,
		ionicAppId: options.ionicKey.ionicAppId
	...
	if (phpjs.intval(this.options.args.feedRate) <= 0) 
		fs.readFile('./data/feedRate', { encoding: 'utf8' }, function (err, data) {
			if (err) {
...
p.fixFeedRate = function(feedRate) {
	...
	this.Queue.fixFeedRate(feedRate);
