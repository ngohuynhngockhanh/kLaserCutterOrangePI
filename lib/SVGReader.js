...
var SVGReader = {
  ...
  tolerance : 0.00095590551,
    // max tollerance when tesselating curvy shapes
  defaultXform: [1,0,0,1,0,0],
  config: {},
  style2object: function(style_data) {
  	var array = phpjs.explode(';', style_data);
  	var result = {};
  	for (var i = 0; i < array.length; i++) {
  		var tmp = phpjs.explode(':', array[i]);
  		result[tmp[0]] = tmp[1];
  	}
  	return result;
  },
  parse : function(svgstring, config) {
    config = config || {};
	this.config = config;
	console.log(config.vbX + ' ' + config.vbY);
    ...
    node.stroke = [255,0,0];
    node.xformToWorld = this.defaultXform

    this.parseChildren(svgRootElement, node)
	if (config.vbX && config.vbY && phpjs.abs(config.vbX - 1) > 0.01 && phpjs.abs(config.vbY - 1) > 0.01)
		for (var i = 0; i < this.boundarys.allcolors.length; i++)
			for (var j = 0; j < this.boundarys.allcolors[i].length; j++) {
				var newX = this.boundarys.allcolors[i][j].x * config.vbX;
				var newY = this.boundarys.allcolors[i][j].y * config.vbY;
				this.boundarys.allcolors[i][j].set(newX, newY);
			}
  ...
  parseChildren : function(domNode, parentNode) {
    ...
    for (var i=0; i<domNode.childNodes.length; i++) {
      ...
      if (tag.childNodes && (!tag.tagName || (tag.tagName && tag.tagName != 'defs'))) {
        if (tag.tagName) {
          ...
          if (tag.tagName == 'text') {//convert text to path
			var tspanList = tag.querySelectorAll('tspan'); 
			for (var j = 0; j < tspanList.length; j++) {
				var tspan = tspanList[j]; 
				var text = tspan.innerHTML;
				if (phpjs.strlen(phpjs.trim(text)) == 0)
					continue;
				var textX = (tspan._attrsByQName.x) ? tspan._attrsByQName.x.data : 0;
				var textY = (tspan._attrsByQName.y) ? tspan._attrsByQName.y.data : 0;
				var style = this.style2object(tag._attrsByQName.style.data);
				var tspanstyle = (tspan._attrsByQName.style) ? this.style2object(tspan._attrsByQName.style.data) : style;
				var fontType = "Normal";
				var fontBold = tspanstyle['font-weight'] == 'bold';
				var fontItalic = tspanstyle['font-style'] == 'italic';
				var fontHeavy = phpjs.intval(tspanstyle['font-weight']) >= 900;
				var fontCondensed = tspanstyle['font-stretch'] == 'condensed';
				if (fontCondensed)
					fontType = 'Condensed'; 
				else if (fontHeavy)
					fontType = 'Heavy';
				...
				var fontSize = (tspanstyle['font-size']) ? phpjs.floatval(tspanstyle['font-size']) : phpjs.floatval(style['font-size']);
				var fontName = phpjs.str_replace(['\'', ' '], '', tspanstyle['font-family']);
				var fontPath = './font/' + fontName + fontType + '.ttf';
				console.log(fontPath);
				if (!fs.existsSync(fontPath))
					fontPath = './font/' + fontName + fontType + '.otf';
					
				if (!fs.existsSync(fontPath))
					fontPath = './font/' + fontName + '.otf';
				
				if (!fs.existsSync(fontPath))
					fontPath = './font/' + fontName + '.ttf';
				
				if (fs.existsSync(fontPath)) {
					var textToSVG = new TextToSVG(fontPath);
					var attributes = {'style': tag._attrsByQName.style.data};
					//add transform
					if (tag._attrsByQName.transform)
						attributes['transform'] = tag._attrsByQName.transform.data; 	
					var options = {x: phpjs.floatval(textX), y: phpjs.floatval(textY), fontSize: fontSize, attributes: attributes};
					 
					var text_svg = textToSVG.getSVG(text, options);
					var dom = require('domino');
					var window = dom.createWindow(text_svg);
					var text_svg_element = window.document.documentElement;
					this.parseChildren(text_svg_element, parentNode);
				}
			}
          } 
          
          if (tag._attrsByQName) {
          	//hot fix for kLaserCutter
          	for (var nodeName in tag._attrsByQName) {
          		var nodeValue = tag._attrsByQName[nodeName].data;
          		if (nodeName && nodeValue && this.SVGAttributeMapping[nodeName]) {
              		//console.log(nodeName + ' ' + nodeValue);
                	this.SVGAttributeMapping[nodeName](this, node, nodeValue)
              	}
              	//
          	}
            //for (var j=0; j<tag.attributes.element.attributes.length; j++) {
            	
            //  var attr = tag.attributes.item(j)
            //  console.log(tag.attributes.element);
